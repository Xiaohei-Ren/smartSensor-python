...
def con_DB(query):
    ...
    :return:
    """
    InfluxClient = InfluxDBClient('121.196.147.234', 8086, 'root', '123456', 'test')
...
def data_reader(id):
    """
    influxdb数据读取 todo:1、influxdb参数查询方式改进 2、时间间隔计算判断未修改
    :param id:
    :return:
    """
...
def data_reader_all(id):
    """
    influxdbq数据读取 全部历史数据
    :param id:
    :return:
    """
    # influxDb查询语句（我崩溃了这个查询语句的引号真的辣鸡，参数查询还用不了）
    if id == 1:
        query = 'select "time","id", "value" from sensor02 where id=1;'
    elif id == 2:
        query = 'select "time","id", "value" from sensor02 where id=2;'
    elif id == 3:
        query = 'select "time","id", "value" from sensor02 where id=3;'
    elif id == 4:
        query = 'select "time","id", "value" from sensor02 where id=4;'
    Data = con_DB(query)
    # print(Data)
    points = Data.get_points()
    time_list = []
...
def temp_line() -> Grid:
    """
    数据处理折线图绘制 todo：多个传感器数据折线图绘制
    :return:
    """
    (time_list_res, value_list_res) = data_reader(1)
    (time_list_res_1, value_list_res_1) = data_reader(2)
    (time_list_res_2, value_list_res_2) = data_reader(3)
    x_data = time_list_res
    y_data = value_list_res
    # print(x_data)
    # print(y_data)
    y_data1 = value_list_res_1
    y_data2 = value_list_res_2
...
def temp_line2() -> Grid:
    ...
    c = (
        Line(init_opts=opts.InitOpts())
            ...
            .add_xaxis(xaxis_data=x_data)
            .add_yaxis(
            series_name="压力传感器PRE001",
            y_axis=y_data,
            symbol="emptyCircle",
            is_symbol_show=False,
            is_smooth=True,
            label_opts=opts.LabelOpts(is_show=False),
            linestyle_opts=opts.LineStyleOpts(width=3, color="#f78f96"),
            # markline_opts=opts.MarkLineOpts(data=[opts.MarkLineItem(y=15, name="温度上限")],
            #                                 linestyle_opts=opts.LineStyleOpts(color="#f9a9af", type_="dashed")),
            # color="orange",
            areastyle_opts=opts.AreaStyleOpts(opacity=0.3, color="#f78f96"),
        )
            #     .add_yaxis(
            #     series_name="温度传感器TS02",
            #     y_axis=y_data1,
...
def liquid_1() -> Grid:
    l1 = (
        Liquid()
            ...
            .set_global_opts(
            title_opts=opts.TitleOpts(title="设备监控", title_textstyle_opts=opts.TextStyleOpts(color="#505458"),
                                      pos_top=0, pos_left=0), )
    )
...
def graph() -> Graph:
    nodes_data = [
        opts.GraphNode(name="基础压力", symbol_size=30, value=70),
        opts.GraphNode(name="加注机", symbol_size=40, value=80),
        opts.GraphNode(name="储氢罐", symbol_size=50, value=90),
        opts.GraphNode(name="压缩机", symbol_size=30, value=70),
        opts.GraphNode(name="长管拖车", symbol_size=40, value=80),
    ]
    links_data = [
        opts.GraphLink(source="基础压力", target="加注机", value=10),
        ...
        opts.GraphLink(source="长管拖车", target="基础压力", value=10),
    ]
    c = (
        Graph()
            .add(
...
class ChartView_1(APIView):
    def get(self, request, *args, **kwargs):
        return JsonResponse(json.loads(temp_line2()))
