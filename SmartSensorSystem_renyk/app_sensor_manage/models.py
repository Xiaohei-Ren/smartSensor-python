...
class Sensor(models.Model):
    ...
    status = models.CharField('当前状态', max_length=128, blank=True)  # 状态（online\offline）
    create_time = models.DateTimeField('创建时间')  # 创建时间
    last_update_time = models.DateTimeField('修改时间', null=True)  # 最后修改时间
    comment = models.CharField('备注', max_length=128, blank=True)  # 备注
    delete = models.BooleanField('删除', default=0)  # 逻辑删除
    unit = models.CharField('单位', max_length=128, blank=True)  # 传感器数值单位
...
class SensorRecord(models.Model):
    ...
    update_time = models.DateTimeField('修改时间')  # 修改时间
    operate = models.IntegerField('操作类型')  # 操作类型(1新增 2修改 3删除)
